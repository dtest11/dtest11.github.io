...
import v2 "github.com/dtest11/alg/lfu_lru/lru/v2"

type LFUCache struct {
	capacity  int
	freqTable map[int]*v2.DoublyLinkedList
	keyTable  map[int]*v2.Element
...
func Constructor(capacity int) LFUCache {
	l := LFUCache{
		capacity:  capacity,
		freqTable: make(map[int]*v2.DoublyLinkedList, capacity),
		keyTable:  make(map[int]*v2.Element, capacity),
	}
	return l
}

// increaseFreq 添加ele 对应的频率，同时更新miniFrequency
func (lfu *LFUCache) increaseFreq(ele *v2.Element, payload Payload) {
	...
	if _, ok := lfu.freqTable[payload.freq]; !ok {
		lfu.freqTable[payload.freq] = v2.NewDoublyLinkedList()
	}
...
func (lfu *LFUCache) Put(key int, value int) {
	...
	var l *v2.DoublyLinkedList
	l, ok := lfu.freqTable[1]
	if !ok {
		l = v2.NewDoublyLinkedList()
		lfu.freqTable[1] = l
	}
